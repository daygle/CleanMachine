...
public sealed partial class SecureDeletePage : Page
{
    ...
    private async void Delete_Click(object sender, RoutedEventArgs e)
    {
        ...
        _cancel = new CancellationTokenSource(); ChooseButton.IsEnabled = false; CancelButton.IsEnabled = true; Progress.Visibility = Visibility.Visible; try { var progress = new Progress<CleanupProgress>(p => { Progress.Value = p.Total == 0 ? 0 : (double)p.Completed / p.Total; StatusText.Text = $"{p.Phase}: {p.Completed}/{p.Total}"; }); var result = await _service.DeleteAsync(_selection, SelectedWipeOptions(), progress, _cancel.Token); StatusText.Text = $"Complete: {result.FilesProcessed} deleted, {result.FilesSkipped} skipped."; } catch (OperationCanceledException) { StatusText.Text = "Secure Delete cancelled."; } catch (Exception ex) { StatusText.Text = ex.Message; } finally { ChooseButton.IsEnabled = true; CancelButton.IsEnabled = false; _cancel.Dispose(); _cancel = null; }
    }
