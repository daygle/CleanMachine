...
public sealed partial class SecureDeletePage : Page
{
    ...
    private async void Delete_Click(object sender, RoutedEventArgs e)
    {
        ...
        _cancel = new CancellationTokenSource(); ChooseButton.IsEnabled = false; CancelButton.IsEnabled = true; Progress.Visibility = Visibility.Visible; UpdateAutoInstaller.IsDestructiveOperationRunning = true; try { var progress = new Progress<CleanupProgress>(p => { Progress.Value = p.Total == 0 ? 0 : (double)p.Completed / p.Total; StatusText.Text = $"{p.Phase}: {p.Completed}/{p.Total}"; }); var result = await _service.DeleteAsync(_selection, SelectedWipeOptions(), progress, _cancel.Token); await new ActivityStore().AddAsync(new ActivityEntry(DateTimeOffset.UtcNow, "Secure Delete", $"Deleted {result.FilesProcessed:N0} file(s), {WindowsCleanupPage.FormatBytes(result.BytesOverwritten)} overwritten, {result.FilesSkipped:N0} skipped.", result.Issues.Take(20).Select(i => $"{i.Path} - {i.Reason}").ToList())); StatusText.Text = $"Complete: {result.FilesProcessed} deleted, {result.FilesSkipped} skipped."; } catch (OperationCanceledException) { await new ActivityStore().AddAsync(new ActivityEntry(DateTimeOffset.UtcNow, "Secure Delete", "Secure Delete cancelled. Some files may already have been permanently deleted.")); StatusText.Text = "Secure Delete cancelled."; } catch (Exception ex) { await new ActivityStore().AddAsync(new ActivityEntry(DateTimeOffset.UtcNow, "Secure Delete", $"Secure Delete failed: {ex.Message}")); StatusText.Text = ex.Message; } finally { UpdateAutoInstaller.IsDestructiveOperationRunning = false; ChooseButton.IsEnabled = true; CancelButton.IsEnabled = false; _cancel.Dispose(); _cancel = null; }
    }
