...
public sealed class SecureDeleteService
{
    ...
    public async Task<SecureDeleteResult> DeleteAsync(IEnumerable<SecureDeleteSelection> selection, SecureDeleteOptions options, IProgress<CleanupProgress>? progress = null, CancellationToken cancellationToken = default)
    {
        ...
        for (var index = 0; index < files.Length; index++)
        {
            ...
            try
            {
                ...
                await OverwriteAsync(path, originalLength, options, cancellationToken);
                if (new FileInfo(path).Length != originalLength)
                    throw new IOException("File size changed during overwrite.");
                File.Delete(path);
                processed++;
                bytes += originalLength;
            }
            catch (IOException ex) { skipped++; issues.Add(new(path, ex.Message)); }
            catch (UnauthorizedAccessException) { skipped++; issues.Add(new(path, "Access denied")); }
    ...
    public Task<SecureDeleteResult> DeleteAsync(IEnumerable<string> files, SecureDeleteOptions options, IProgress<CleanupProgress>? progress = null, CancellationToken cancellationToken = default)
        => DeleteAsync(files.Select(path => new SecureDeleteSelection(path, 0, true)), options, progress, cancellationToken);
    ...
    {
        if (method == WipeMethod.SimpleZeroFill
            || (method == WipeMethod.Dod522022M && pass == 2)
            || (method == WipeMethod.Dod522022MEce && pass is 2 or 5))
        {
            Array.Clear(buffer, 0, count);
            return;
        }
        if (method is WipeMethod.Dod522022M or WipeMethod.Dod522022MEce)
        {
            buffer.AsSpan(0, count).Fill(pass % 2 == 0 ? (byte)0xFF : (byte)0x00);
            return;
        }
