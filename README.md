...
### Registry Care
- Read-only scanning of current-user (HKCU) entries only - uninstall metadata, file associations, MUI cache, startup entries, orphaned sound events, Shell app-name cache (MuiCache) for missing programs, and App Paths entries pointing at missing programs. Machine-wide (HKLM) keys are never touched, since editing them needs elevation and is far riskier
